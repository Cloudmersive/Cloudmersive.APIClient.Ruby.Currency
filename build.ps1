...
& java -jar swagger-codegen-cli-2.4.14.jar generate -i https://api.cloudmersive.com/swagger/api/currency -l ruby -o client -c packageconfig.json
...
(Get-Content ./client/cloudmersive-currency-api-client.gemspec).replace('`find *`.split("\n").uniq.sort.select { |f| !f.empty? }', "Dir['./**/*']") | Set-Content ./client/cloudmersive-currency-api-client.gemspec
#& npm build ./client
